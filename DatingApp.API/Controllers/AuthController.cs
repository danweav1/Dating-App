...
namespace DatingApp.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register(UserForRegisterDto userForRegisterDto)
        {
            ...
            if (result.Succeeded)
            {
                return CreatedAtRoute("GetUser", new { controller = "Users", id = userToCreate.Id }, userToReturn); // the location of where to get the newly created entity (the new user)
            }
        ...
        public async Task<IActionResult> Login(UserForLoginDto userForLoginDto)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return Ok(new
                {
                    token = GenerateJwtToken(user).Result, // write our token into response we're sending back to client
                    user = appUser
                });
            }
